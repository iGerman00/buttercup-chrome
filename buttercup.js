  ...
  const interval = setInterval(() => {
    if (window['ytInitialPlayerResponse']) {
      if (window['ytInitialPlayerResponse'].captions) {
        console.log('[Buttercup]', window['ytInitialPlayerResponse'].captions)
  ...
  function CustomXMLHttpRequest() {
    ...
    Object.defineProperty(xhr, "responseText", {
      get: function () {
        ...
        if (
          (this.responseURL.includes('/api/timedtext') || isButtercup)
          && isPlayer()
        ) {
    ...
    const observer = new MutationObserver(function (mutationsList, observer) {
      ...
      if (menu) {
        ...
        menuItem.addEventListener("click", function () {
          ...
          chrome.storage.local.set({ 'buttercup_translate': TRANSLATE });
          menuItem.setAttribute("aria-checked", TRANSLATE);
        });
      }
    });
